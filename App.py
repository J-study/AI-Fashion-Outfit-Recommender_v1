...
with st.sidebar:
    with st.expander("Model Settings", expanded=False):
        st.info("LoRAは外部ストレージまたはHF Hubに配置")
        
        pipe_path = st.text_input("SD model path", "")
        
        lora_repo_id = st.text_input(
            "LoRA Hugging Face Repo ID",
            "yutakun1/Normalfashion_LoRA", 
        )
            ...
            with st.spinner("Initializing models... (This may take several minutes on CPU)"):
                try:
                    init_sd(
                        ...
                        lora_repo_id=lora_repo_id if lora_repo_id else None,
                        lora_weight_name=lora_weight_name if lora_weight_name else None,
                    )
